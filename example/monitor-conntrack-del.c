/**
 * nanonl: monitor-ct-del Monitor conntrack deletions
...
int main(void)
{
	...
	if (nl_multicast(fd, NL_MULTICAST_JOIN,
		...
		perror("Unable to join the ctdestroy group");
		goto ret;
...
loop:
	...
	for (e = m; NLMSG_OK(e, len); e = NLMSG_NEXT(e, len)) {
		...
		if ((e->nlmsg_type & 0xff) != IPCTNL_MSG_CT_DELETE)
			continue;
